  ...
  const debounced = useMemo(() => {
    ...
    const func: DebouncedState<T> = (...args: Parameters<T>): ReturnType<T> => {
      ...
      if (flushOnExit && !visibilityListener.current) {
        ...
        globalThis.document?.addEventListener?.('visibilitychange', visibilityListener.current)
      }
      if (isInvoking) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (visibilityListener.current) {
        globalThis.document?.removeEventListener?.('visibilitychange', visibilityListener.current)
